...
// Generate Payment Number (PAY-YYYYMM-0001)
const generatePaymentNumber = async () => {
  ...
  const lastPayment = await Payment.findOne({
    payment_number: new RegExp(`^${prefix}`)
...
export const recordPayment = async (req, res) => {
  try {
    ...
    // Map payment_method to payment_mode (API field to model field)
    ...
    const payment_mode = paymentModeMap[payment_method] || "BANK_TRANSFER";

    // Create payment record with all required fields
    const payment = new Payment({
      payment_number,
      payment_date: new Date(), // Required field
      invoice_id,
      invoice_number: invoice.invoice_number,
      hospital_id: invoice.hospital_id, // Required field - from invoice
      hospital_name: invoice.hospital_name,
      amount_received,
      payment_mode, // Required field - mapped from payment_method
      reference_number,
      notes,
      status: "RECEIVED",
...
// Generate Payout Number (DP-YYYYMM-0001)
const generatePayoutNumber = async () => {
  ...
  const lastPayout = await DoctorPayout.findOne({
    payout_number: new RegExp(`^${prefix}`)
  }).sort({ payout_number: -1 });
  
  let sequence = 1;
  if (lastPayout) {
    const lastSequence = parseInt(lastPayout.payout_number.split('-')[2]);
    sequence = lastSequence + 1;
  }
...
export const createDoctorPayout = async (req, res) => {
  try {
    ...
    if (!doctor_id || !gross_amount || !tds_amount || !net_amount) {
      return res.status(400).json({
        success: false,
        message: "Missing required fields: doctor_id, gross_amount, tds_amount, net_amount"
      });
    }
    ...
    // Map payment_method to payout_mode (API field to model field)
    const payoutModeMap = {
      "Bank Transfer": "BANK_TRANSFER",
      "Cheque": "CHEQUE",
      "Cash": "CASH",
    ...
    const payout_mode = payoutModeMap[payment_method] || "BANK_TRANSFER";

    // Create payout record with all required fields
    const payout = new DoctorPayout({
      payout_number, // Required field
      payout_date: payout_date ? new Date(payout_date) : new Date(), // Required field
      invoice_ids: invoice_id ? [invoice_id] : [], // Can be empty array
      doctor_id, // Required field
      doctor_name,
      gross_amount, // Required field
      tds_amount, // Required field
